...
**Unit testing:** Process in which the smallest testable parts of an application, called units, are individually and independently scrutinized for proper operation.

Benefits

- Guard against breaking changes. (If we upgraded the app)
- Analyze code behavior (expected and unexpected)
- Reveal design mistakes

To test the angular project `ng test`
...
## Nx workspaces (Javascript agnostic workspace)

`nx generate @nrwl/react:app`
Generate is the command name
@nrwl/react is the plugin or the collection name
app is the schematic name
